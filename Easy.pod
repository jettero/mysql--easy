...
MariaDB::Easy - Perl extension to handle various mundane DBI session related things specific to mysql.

=head1 SYNOPSIS

  use MariaDB::Easy;

  my $dbo = MariaDB::Easy->new("stocks");
...
Here are the functions L<MariaDB::Easy> provides:

=over

=item B<new>

    $dbo = MariaDB::Easy->new( $db_name, $trace )
...
the L<< DBI->trace()|DBI/trace >> ...
    ...
    $dbo = MariaDB::Easy->new( $db_name, {user=>"blarg", host=>"whatever"})
...
L<MariaDB::Easy> uses only write locks.  Those are the ones where nobody can read
...
mostly.  The L<MariaDB::Easy> statement handles automatically recover from
...
Fetches the last insert id (relating to auto_increment fields) from MariaDB.
...
The first time you do a L</do>/L</ready>/L</firstcol>/etc, L<MariaDB::Easy>
...
L<MariaDB::Easy> reads the user and pass from that file.  The host name will
...
L<MariaDB::Easy> actually checks a few places for the username and password
